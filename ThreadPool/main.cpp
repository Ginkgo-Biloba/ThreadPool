...
public:
	...
	void call(uint32_t from, uint32_t to) override
	{
...
int main()
{
	...
	int const TRD[] = {6, 1, 0, 8, 1, 16, 7, 4};
...

	{
		...
		for (int i = 0; i < nTRD; ++i) {
			fprintf(stdout, " sync set thread to %2d", TRD[i]);
		...
		for (int i = 0; i < 7; ++i) {
			...
			fprintf(stdout,
				" sync %d x % 9.6f y % 9.7f r %9.7f t %9.3f\n",
				i, x, y, r, elapse);
		}
	}

	{
		AsyncPool pool;
		for (int i = 0; i < nTRD; ++i) {
			fprintf(stdout, "async set thread to %2d", TRD[i]);
			pool.setNumThread(TRD[i]);
			fprintf(stdout, " done\n");
			Sleep(100);
		}
...

	{
		...
		for (int i = 0; i < 7; ++i) {
			...
			if (i > 0)
				x = X0, y = Y0, r = pow(0.2, i);
			int64_t t1 = getTickCount();
			RefPtr<MbAsync> mb = new MbAsync(ntrd, rows, cols, x, y, r);
			...
			double elapse = static_cast<double>(getTickCount() - t1) * ifreq;
			fprintf(stdout,
				"async %d x % 9.6f y % 9.7f r %9.7f t %9.3f\n",
				i, x, y, r, elapse);
		}
	}
}
